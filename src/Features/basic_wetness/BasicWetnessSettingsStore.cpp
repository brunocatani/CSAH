...
namespace community_shaders::basic_wetness
{
    namespace
    {
        constexpr auto kSection = L"BasicWetness";
    ...
    Settings loadSettings(const std::filesystem::path& path) noexcept
    {
        ...
        return master_settings::gate(path, sanitize({
            ...
            .wetness = readFloat(path, L"fWetness", defaults.wetness),
            .diffuseDarkening = readFloat(
                path, L"fDiffuseDarkening", defaults.diffuseDarkening),
            .specularMultiplier = readFloat(
                path, L"fSpecularMultiplier", defaults.specularMultiplier),
            .roughnessScale = readFloat(
                path, L"fRoughnessScale", defaults.roughnessScale),
        }));
    }
    ...
    Settings loadSettings() noexcept
    {
        ...
        logging::info(
            "Basic Wetness settings loaded from '{}'; enabled={}, wetness={}, darkening={}, specular={}, roughnessScale={}.",
            path.string(), result.enabled, result.wetness,
            result.diffuseDarkening, result.specularMultiplier,
            result.roughnessScale);
        return result;
    }
    ...
    bool saveSettings(const Settings& settings) noexcept
    {
        ...
        auto success = WritePrivateProfileStringW(
                           ...
                           path.c_str()) != FALSE;
        success = writeFloat(path, L"fWetness", safe.wetness) && success;
        success = writeFloat(
                      path, L"fDiffuseDarkening", safe.diffuseDarkening) &&
            success;
        success = writeFloat(
                      path, L"fSpecularMultiplier", safe.specularMultiplier) &&
            success;
        success = writeFloat(
                      path, L"fRoughnessScale", safe.roughnessScale) &&
            success;
        return success;
    }
}
