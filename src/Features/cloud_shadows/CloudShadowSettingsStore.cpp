...
namespace community_shaders::cloud_shadows
{
    namespace
    {
        constexpr auto kSection = L"CloudShadows";
    ...
    Settings loadSettings() noexcept
    {
        ...
        logging::info(
            "Cloud Shadows settings loaded from '{}'; enabled={}, opacity={}.",
            path.string(), result.enabled, result.opacity);
        return result;
    }
    ...
    bool saveSettings(const Settings& settings) noexcept
    {
        ...
        auto success = WritePrivateProfileStringW(
                           ...
                           path.c_str()) != FALSE;
        success = writeFloat(path, L"fOpacity", safe.opacity) && success;
        return success;
    }
}
