...
namespace
{
    ...
    constexpr Pixel kSource{ 0.42F, 0.63F, 0.31F, 0.77F };
    constexpr Pixel kGrayscale{ 0.18F, 0.36F, 0.72F, 0.54F };
    constexpr Pixel kVertexColor{ 0.65F, 0.8F, 0.45F, 0.7F };
    constexpr float kColorScale = 1.35F;

    struct alignas(16) ParticlePerGeometry
    {
        float colorScale{};
        float padding[3]{};
    };
    ...
    constexpr std::array<Contract, 4> kContracts{ {
        { "ParticleDefault_00000000", false, false },
        { "ParticleGrayscaleToColor_00000001", true, false },
        { "ParticleGrayscaleToAlpha_00000002", false, true },
        { "ParticleGrayscaleToColorAlpha_00000003", true, true },
    } };
...
VSOutput VSMain(uint vertexId : SV_VertexID)
{
    ...
    return output;
        ...
        const auto result = D3DCompile(
            source,
            sizeof(source) - 1,
            "ParticleLinearLightingParityVS",
            nullptr,
            nullptr,
            "VSMain",
            "vs_5_0",
    ...
    Pixel render(
        ...
        const std::array<ID3D11ShaderResourceView*, 2>& textures,
        const std::array<ID3D11SamplerState*, 2>& samplers)
    {
        ...
        context->PSSetConstantBuffers(2, 1, &particleBuffer);
        ...
        const std::array<ID3D11ShaderResourceView*, 2> nullTextures{};
        context->PSSetShaderResources(
    ...
    Pixel expectedVanilla(const Contract& contract)
    {
        Pixel result{};
        for (std::size_t channel = 0; channel < 3; ++channel) {
            result[channel] = (contract.grayscaleColor ?
                    kGrayscale[channel] :
                    kSource[channel] * kVertexColor[channel]) *
                kColorScale;
        }
        result[3] = contract.grayscaleAlpha ?
            kGrayscale[3] :
            kSource[3] * kVertexColor[3];
        return result;
    }

    Pixel expectedEnabled(const Contract& contract, const Settings& settings)
    {
        Pixel result = expectedVanilla(contract);
        for (std::size_t channel = 0; channel < 3; ++channel) {
            const float baseColor = contract.grayscaleColor ?
                kGrayscale[channel] :
                kSource[channel] * kVertexColor[channel];
            result[channel] =
                std::pow(std::abs(baseColor), settings.colorGamma) *
                settings.vanillaDiffuseColorMultiplier * kColorScale;
        }
        return result;
    }
    ...
    int run(const std::filesystem::path& root)
    {
        ...
        const ParticlePerGeometry particleConstants{ kColorScale, {} };
        const auto particleBuffer =
            createConstantBuffer(device.Get(), particleConstants);

        Settings disabledSettings{};
        disabledSettings.enabled = false;
        Settings enabledSettings = disabledSettings;
        enabledSettings.enabled = true;
        enabledSettings.colorGamma = 1.75F;
        ...
        const auto sourceTexture = createTexture(device.Get(), kSource);
        const auto grayscaleTexture = createTexture(device.Get(), kGrayscale);
        const std::array<ID3D11ShaderResourceView*, 2> textures{
            sourceTexture.Get(), grayscaleTexture.Get()
        };
        ...
        for (const auto& contract : kContracts) {
            const auto vanillaShader = createPixelShader(
                device.Get(),
                root /
                    "package/Shaders/Community/VerifiedParticleLinearLighting" /
            ...
            const auto vanilla = render(
                ...
                vanillaShader.Get(), particleBuffer.Get(),
                disabledFrameBuffer.Get(), textures, samplers);
            const auto disabled = render(
                device.Get(), context.Get(), vertexShader.Get(),
                replacementShader.Get(), particleBuffer.Get(),
                disabledFrameBuffer.Get(), textures, samplers);
            const auto enabled = render(
                device.Get(), context.Get(), vertexShader.Get(),
                replacementShader.Get(), particleBuffer.Get(),
                enabledFrameBuffer.Get(), textures, samplers);

            passed &= compare(
                vanilla,
                expectedVanilla(contract),
                std::string(contract.name) + " vanilla model");
            passed &= compare(
                disabled,
                vanilla,
                std::string(contract.name) + " disabled parity");
            passed &= compare(
                enabled,
                expectedEnabled(contract, enabledSettings),
                std::string(contract.name) + " enabled model");
        }
        if (!passed) {
            return 1;
        }
        std::cout <<
            "Particle Linear Lighting disabled parity and enabled model tests passed for all four contracts.\n";
        return 0;
    }
}
...
int main(int argumentCount, char** arguments)
{
    if (argumentCount != 2) {
        std::cerr <<
            "usage: ParticleLinearLightingShaderParityTests <repo-root>\n";
        return 2;
    }
    try {
        return run(std::filesystem::path(arguments[1]));
    } catch (const std::exception& error) {
        std::cerr << "Particle Linear Lighting parity test failed: "
                  << error.what() << '\n';
        return 1;
    }
}
